...
namespace RT_ISICG
{
	class BaseMaterial
	{
	  public:
		...
		virtual inline bool const	isMirror() const { return false; }
		virtual inline bool const	isTransparent() const { return false; }
		virtual inline float const	getIor() const { return 1.f; }
	  protected:
		std::string _name;
	};
